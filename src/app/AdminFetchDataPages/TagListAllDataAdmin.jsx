...
const AuthorAllDataAdmin = () => {
    ...
    async (isBackgroundRefresh = false) => {
      try {
        ...
        const response = await axios.get(
          `http://localhost:5001/api/admin/all-tag?${params.toString()}`
        );
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await axios.delete(
        `http://localhost:5001/api/admin/delete/tag/${id}`
      );
  ...
  return (
    <div className="fontPoppins border border-gray-400 rounded-xl p-2">
      {/* Filter Form */}
      {isRefreshing && (
