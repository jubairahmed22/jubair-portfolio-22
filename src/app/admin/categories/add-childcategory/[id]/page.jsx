...
const EditChildCategoryPage = () => {
  ...
  const childCategoryId = params.id;
    ...
    const fetchSubCategories = async () => {
      ...
      try {
        ...
        const filteredSubCategories = response.data.products.filter(
          (subCat) => subCat.parentCategory.id === selectedCategoryId
        );
  ...
  useEffect(() => {
    const fetchChildCategory = async () => {
      try {
        setIsLoading(true);
        const response = await axios.get(
          `http://localhost:5001/api/admin/child-category/${childCategoryId}`
        );
        const childCategory = response.data;

        setFormData({
          title: childCategory.title,
          showWebsite: childCategory.showWebsite,
        });

        setExistingImage(childCategory.singleImage || "");

        // Set parent category ID directly
        if (childCategory.parentCategory?.id) {
          setSelectedCategoryId(childCategory.parentCategory.id);
        }

        // Set parent sub-category ID directly
        if (childCategory.parentSubCategory?.id) {
          setSelectedSubCategoryId(childCategory.parentSubCategory.id);
        }
      } catch (err) {
        setError(err.message);
        Swal.fire(
          "Error!",
          err.response?.data?.error || "Failed to fetch child category details",
          "error"
        );
      } finally {
        setIsLoading(false);
...
const handleSubmit = async (e) => {
  ...
  try {
    ...
    // Add sub-category reference if selected
    if (selectedSubCategoryId) {
      const selectedSubCategory = subCategories.find(sub => sub._id === selectedSubCategoryId);
      formDataToSend.append("subCategoryId", selectedSubCategoryId);
    ...
    const response = await axios.put(
      `http://localhost:5001/api/admin/update/child-categoryyy/${childCategoryId}`,
      formDataToSend,
      {
        headers: {
    ...
    Swal.fire(
      ...
      router.push("/admin/categories/add-childcategory");
    });
  } catch (err) {
    setError(err.message);
    Swal.fire(
      "Error!",
      err.response?.data?.error || "Failed to update child category",
      "error"
    );
  } finally {
    setIsLoading(false);
